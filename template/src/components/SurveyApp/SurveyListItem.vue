    ...
    <b-card no-body style="margin:5px;">
        ...
        <div class="d-flex flex-grow-1 min-width-zero">
            <b-card-body class="align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center pb-2">
                ...
                <span v-if="data.notifType=='LC'" class="align-middle d-inline-block">{{data.senderUsername}} liked your comment "{{data.commentText}}".  </span>
                <span v-if="data.notifType=='CR'" class="align-middle d-inline-block">{{data.senderUsername}} replied your comment "{{data.commentText}}".  </span>
                <p class="mb-1 text-muted text-small w-15 w-xs-100">{{convertFromStringToDate(data.date)}}</p>
            </b-card-body>
